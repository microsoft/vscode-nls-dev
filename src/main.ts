				...
				if (!reference.isWriteAccess) {
					...
					if (isIdentifier(node)) {
						...
						if (isCallExpression(parent) && parent.arguments.length >= 2) {
							...
							errors.push(`(${position.line},${position.character}): localize function (bound to ${node.text}) used in an unusal way.`);
						}
					}
				}
			});
...
export function process(contents: string, sourceMapContents: string): { contents: string, sourceMap: string, errors: string[] } {

	const analysisResult = analyze(contents);
	if (analysisResult.patches.length === 0) {
		return undefined;
	}
	let rawSourceMap: RawSourceMap = JSON.parse(sourceMapContents);
